# -*- coding: utf-8 -*-
"""
Created on Fri May 17 16:52:43 2024
GRU - CIFAR10
...
from keras.layers import GRU, RepeatVector, TimeDistributed, Dense, Flatten, Reshape
...
data = data_flat.reshape(data.shape[0], 32, 32, 3)  # Reshape back to original dimensions

# Reshape to (samples, time_steps, features) for GRU
data = data.reshape(data.shape[0], 32, -1)  # Each image row is treated as a sequence
...
# Define a GRU autoencoder model
def create_gru_autoencoder(input_shape):
    model = Sequential()
...
models = [create_gru_autoencoder(input_shape) for _ in range(num_clusters)]
